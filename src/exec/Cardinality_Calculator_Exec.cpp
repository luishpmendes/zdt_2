...
int main(int argc, char * argv[]) {
    ...
    for(unsigned i = 0; i < numSolvers; i++) {
        if(!argParser.cmdOptionExists("--cardinality-" + std::to_string(i))) {
            break;
        }

        std::string cardinalityFilename =
            argParser.getCmdOption("--cardinality-" + std::to_string(i));

        std::ofstream ofs;
        ofs.open(cardinalityFilename);

        if(ofs.is_open()) {
            ofs << paretos[i].size() << std::endl;
        } else {
    ...
    for(unsigned i = 0; i < numSolvers; i++) {
        std::string capturedCardinalitiesFilename =
            argParser.getCmdOption("--captured-cardinalities-"
                                   + std::to_string(i));

        std::ofstream ofs;
        ofs.open(capturedCardinalitiesFilename);
        ...
        if(ofs.is_open()) {
            for(unsigned j = 0;
                ...
                ofs << capturedIterations[i][j] << ","
                    << capturedTimes[i][j] << "," 
                    << capturedParetos[i][j].size() << std::endl;
            }
        } else {
            throw "File not created.";
        }
